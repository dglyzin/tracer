...
def run():
    ...
    f = np.vectorize(lambda xl, yl: u((xl, yl))[0])
    g = np.vectorize(lambda xl, yl: u((xl, yl))[1])
    print("xu.max:")
    print(xu.max())
    print("yu.max:")
    print(yv.max())
    ...
    result_u = f(xu, yv)
    result_v = g(xu, yv)
    return((u_, p_, mesh, result_u, result_v))


if __name__ == '__main__':
    u, p, mesh, result_u, result_v = run()

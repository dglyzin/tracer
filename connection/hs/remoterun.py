...
def remoteProjectRun(connection, inputFile, params, projectFolder, logger):
    ...
    try:
        ...
        logger.log("Checking if solver executable at "
                   + connection.tracerFolder
                   + "/hybridsolver/bin/HS"
                   + " exists...", LL_USER)

        stdin, stdout, stderr = client.exec_command('test -f '
                                                    + connection.tracerFolder
                                                    + "/hybridsolver/bin/HS")
        if stdout.channel.recv_exit_status():
            logger.log("Please provide correct path to"
                       + " the solver executable.", LL_USER)
            return
        else:
        ...
        stdin, stdout, stderr = client.exec_command('sh ' + projFolder
                                                    + '/'+remoteRunScriptName
                                                    + " 2>&1")

...
void Block0Bound0_1(double* result, double* source, double t, int idxX, int idxY, double* params, double** ic){
    ...
    double nonexistent;
    bound_value = $@nei1@$;
    nonexistent =       source[idx-Block0StrideY*CELLSIZE] - 2.0 * bound_value * DY2;
	$@sistemGy1@$
    bound_value = $@nei2@$;
    nonexistent =    source[idx-Block0StrideY*CELLSIZE + 1] - 2.0 * bound_value * DY2;
	$@sistemGy2@$
}
//угловая y=ymax

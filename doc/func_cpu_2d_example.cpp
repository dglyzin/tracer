...
void Initial4(double* cellstart, double x, double y, double z){
    cellstart[0] = -10.0;
}
...
void Block0DefaultNeumannBound0(double* result, double* source, double t, int idxX, int idxY, int idxZ, double* params, double** ic){
	 int idx = ( idxX + idxY * Block0StrideY + idxZ * Block0StrideZ) * Block0CELLSIZE;
	 result[idx + 0] = (2.0 * DXM2 * (source[idx + Block0StrideX * Block0CELLSIZE + 0] - source[idx + 0] - (0) * DX)) + (DYM2 * (1 * source[idx + 1 * Block0StrideY * Block0CELLSIZE + 0] - 2 * source[idx + 0 * Block0StrideY * Block0CELLSIZE + 0] + 1 * source[idx-1 * Block0StrideY * Block0CELLSIZE + 0]));
}
...
void getBlock0BoundFuncArray(func_ptr_t** ppBoundFuncs){
    ...
    pBoundFuncs = (func_ptr_t*) malloc( ( (1 + 8) * 1) * sizeof(func_ptr_t) );
